   ...
   public class Network
   {
      ...
      public function send(param1:AbstractPacket) : void
      {
         ...
         if(param1 != null && param1.getId() == SelectBattleInOutPacket.id && BattleSelectionTrace.ENABLED)
         {
            ...
            BattleSelectionTrace.record("NETWORK_SOCKET_STATE","Network.send",_loc2_,null,"connected=" + (this.socket != null && this.socket.connected) + " sendBufferLength=" + this.sendBuffer.length + " sendBufferPosition=" + this.sendBuffer.position + " stringLength=" + _loc2_.length + " trailingCodes=" + this.getTrailingCharacterCodes(_loc2_) + " trailingAsciiSpace=" + (_loc2_.length > 0 && _loc2_.charCodeAt(_loc2_.length - 1) == 32));
            if(this.socket == null || !this.socket.connected)
            {
               BattleSelectionTrace.record("NETWORK_SEND_SKIPPED","Network.send",_loc2_,null,"socketDisconnected");
               return;
            }
            try
            {
            ...
            {
               BattleSelectionTrace.record("NETWORK_SEND_ERROR","Network.send",_loc2_,null,"type=" + e.name + " message=" + e.message + " stack=" + e.getStackTrace() + " bufferLength=" + this.sendBuffer.length + " bufferPosition=" + this.sendBuffer.position);
               throw e;
            }
            finally
            {
         ...
         {
            if(param1.getId() == 2092412133 || param1.getId() == -1284211503)
            {
