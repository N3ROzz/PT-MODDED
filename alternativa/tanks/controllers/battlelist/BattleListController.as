package alternativa.tanks.controllers.battlelist
{
   ...
   public class BattleListController extends EventDispatcher implements IBattleListViewControllerCallback
   {
      ...
      private static const DEFAULT_MODE:BattleMode = null;
      ...
      private static function storeMode(param1:BattleMode) : void
      {
         if(param1 == null)
         {
            delete storageService.getStorage().data[MODE_FILTER];
         }
         else
         {
      ...
      {
         return this.filteredMode == null || param1 == this.filteredMode;
      }
      ...
      {
         if(this.filteredMode == param1 && !param2)
         {
            this.showAllBattles();
            battleSelectActionsService.turnOffBattleMode(param1);
            this.filteredMode = null;
            storeMode(null);
         }
         else if(this.filteredMode != param1 && param2)
         {
            if(this.filteredMode == null)
            {
               this.hideBattlesExceptType(param1);
            }
            else
            {
      ...
      private function showAllBattles() : void
      {
         var _loc2_:int = int(this._allItems.length);
         var _loc3_:int = 0;
         while(_loc3_ < _loc2_)
         {
            if(this._view.getItemIndex(this._allItems[_loc3_].id) < 0)
            {
               this._view.createItem(this._allItems[_loc3_],false);
            }
            _loc3_++;
         }
         this._view.sortBattleList();
      ...
      private function hideBattlesExceptType(param1:BattleMode) : void
      {
         var _loc2_:int = 0;
         var _loc3_:int = int(this._allItems.length);
         var _loc4_:int = 0;
