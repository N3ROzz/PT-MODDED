package alternativa.tanks.models.tank.ultimate.hornet.radar
{
   ...
   public class BattleRadarHudIndicators implements Renderer
   {
      ...
      private static const MARKER_SCALE:Number = 0.9;
      ...
      {
         var _loc2_:PointHudIndicator = new PointHudIndicator(this.getIndicatorBitmap(param1),param1);
         _loc2_.filters = [new GlowFilter(this.getTeamColor(param1),0.45,5,5,1.5,2,false)];
         _loc2_.scaleX = MARKER_SCALE;
         _loc2_.scaleY = MARKER_SCALE;
         return _loc2_;
      }
      ...
      private function getIndicatorBitmap(param1:Tank) : Bitmap
      {
         ...
         {
            case BattleTeam.RED:
               _loc2_ = new Bitmap(this.battleUltimateRadarCC.redTankMarker);
               break;
            case BattleTeam.BLUE:
               _loc2_ = new Bitmap(this.battleUltimateRadarCC.blueTankMarker);
               break;
            case BattleTeam.NONE:
            default:
               _loc2_ = new Bitmap(this.battleUltimateRadarCC.neutralTankMarker);
         }
      ...
      private function updateMarker(param1:Matrix4, param2:PointHudIndicator, param3:Tank) : void
      {
         ...
         if(!this.shouldShowTank(param3) || !param2.isActive(this.camera) || this.getBossRelationRole(param3) == BossRelationRole.BOSS)
         {
         ...
         if(v.z > 0 && _loc6_)
         {
            ...
            else
            {
               param2.visible = true;
               param2.alpha = _loc8_;
               param2.scaleX = MARKER_SCALE;
               param2.scaleY = MARKER_SCALE;
            }
         }
         else
         {
         ...
         param2.x = v.x + _loc7_.getWidth() / 2;
         param2.y = v.y + _loc7_.getHeight() / 2;
         param2.zindex = this.getDistanceToCamera(param2);
      }

      private function shouldShowTank(param1:Tank) : Boolean
      {
         ...
         return WallHackSystem.shouldRevealTank(_loc2_,param1);
      }

      private function getTeamColor(param1:Tank) : uint
      {
         switch(param1.teamType)
         {
            case BattleTeam.BLUE:
