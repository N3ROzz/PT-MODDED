package alternativa.tanks.battle.objects.tank
{
   ...
   public class Tank implements PhysicsController, PostPhysicsController, PhysicsInterpolator, Renderer, BodyCollisionFilter, CameraTarget, WeaponPlatform, PointIndicatorStateProvider
   {
      ...
      // Basic AI bridge: read-only weapon type check plus normal local trigger control.
      public function isFirebirdWeaponForBasicAI() : Boolean
      {
         return this.weapon is StreamWeapon && StreamWeapon(this.weapon).getResistanceProperty() == ItemProperty.FIREBIRD_RESISTANCE;
      }
      
      public function pullTriggerForBasicAI() : void
      {
         var model:ITankModel = null;
         var controller:LocalWeaponController = null;
         if(this.getUser() != null)
         {
            model = ITankModel(this.getUser().adapt(ITankModel));
            if(model != null && model.getWeaponController() is LocalWeaponController)
            {
               controller = LocalWeaponController(model.getWeaponController());
               controller.pullTrigger();
            }
         }
      }
      
      public function releaseTriggerForBasicAI() : void
      {
         var model:ITankModel = null;
         var controller:LocalWeaponController = null;
         if(this.getUser() != null)
      ...
      public function setTitleDepthTest(param1:Boolean) : void
      {
         this.title.setDepthTest(param1);
      }
      
      public function setTitleConfiguration(param1:int) : void
      {
         this.title.setConfiguration(param1);
      }
      
      public function setTitleHealth(param1:int) : void
      {
         this.title.setHealth(param1);
      }

      public function setTitleLabelText(param1:String) : void
      {
         this.title.setLabelText(param1);
      }
