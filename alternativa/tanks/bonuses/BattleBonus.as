package alternativa.tanks.bonuses
{
   ...
   public class BattleBonus extends PooledObject implements PhysicsController, PhysicsInterpolator, Renderer, Bonus, ISpawnableObject
   {
      ...
      public function init(param1:Long, param2:Long, param3:BattleBonusData, param4:BattleService) : void
      {
         ...
         this.trigger.resetDiagnosticsState();
      }
      ...
      {
         var _loc6_:String = GoldBoxDiagnostics.canonicalFromLong(this._bonusId);
         ...
         {
            GoldBoxDiagnostics.lifecycle(_loc6_,"SPAWN_BRANCH_ON_GROUND","SPAWNING_ON_GROUND");
            this.initOnGround(P,N);
         }
         else
         {
            GoldBoxDiagnostics.lifecycle(_loc6_,"SPAWN_BRANCH_AIRBORNE","FALLING");
      ...
      private function initOnGround(param1:Vector3, param2:Vector3) : void
      {
         ...
         GoldBoxDiagnostics.captureBonusPosition(GoldBoxDiagnostics.canonicalFromLong(this._bonusId),_bonusPosition.x,_bonusPosition.y,_bonusPosition.z,true);
      ...
      {
         var _loc1_:GroundSpawnRenderer = GroundSpawnRenderer(this.battleService.getObjectPool().getObject(GroundSpawnRenderer));
         _loc1_.start(this);
      }
      ...
      {
         // Random angle
         return Math.PI * 10 * origin.x * origin.y / 180;
      }
      ...
      private function initAirborne(param1:Vector3, param2:Vector3, param3:Vector3, param4:int) : void
      {
         ...
         else
         {
            ...
            if(Boolean(this.battleService.getBattleRunner().getCollisionDetector().raycastStatic(origin,Vector3.DOWN,CollisionGroup.STATIC,BIG_VALUE,null,_rayHit)))
            {
         ...
         {
            GoldBoxDiagnostics.lifecycle(GoldBoxDiagnostics.canonicalFromLong(this._bonusId),"FALLING_STARTED","FALLING");
            this.initParachute();
            this.addAllToScene();
      ...
      {
         var _loc2_:CollisionDetector = this.battleService.getBattleRunner().getCollisionDetector();
         return _loc2_.hasStaticHit(param1,Vector3.Z_AXIS,CollisionGroup.STATIC,BIG_VALUE);
      }
      
      private function getGroundPointAndNormal(param1:Vector3, param2:Vector3, param3:Vector3) : void
      {
         var _loc4_:CollisionDetector = this.battleService.getBattleRunner().getCollisionDetector();
         if(_loc4_.raycastStatic(param1,Vector3.DOWN,CollisionGroup.STATIC,BIG_VALUE,null,_rayHit))
         {
      ...
      {
         GoldBoxDiagnostics.lifecycle(GoldBoxDiagnostics.canonicalFromLong(this._bonusId),"PICKUP_BEGIN","PICKUP");
      ...
      private function playPickupSound() : void
      {
         ...
         {
            _loc1_ = Sound3D.create(this.data.pickupSound,0.5);
            this.bonusMesh.readPosition(_bonusPosition);
            this.battleService.addSound3DEffect(Sound3DEffect.create(_bonusPosition,_loc1_));
         }
      }
      
      private function startPickupAnimation() : void
      {
         var _loc1_:BonusPickupAnimation = BonusPickupAnimation(this.battleService.getObjectPool().getObject(BonusPickupAnimation));
      ...
      {
         GoldBoxDiagnostics.lifecycle(GoldBoxDiagnostics.canonicalFromLong(this._bonusId),"REMOVE_BEGIN","REMOVING");
         this.onRemove.dispatch();
         this.bonusMesh = null;
         this.destroy();
      }
      ...
      {
         var _loc1_:String = GoldBoxDiagnostics.canonicalFromLong(this._bonusId);
         ...
         this.data = null;
         GoldBoxDiagnostics.destroyed(_loc1_);
         recycle();
      }
      ...
      {
         var _loc1_:RemovalAnimation = RemovalAnimation(this.battleService.getObjectPool().getObject(RemovalAnimation));
         _loc1_.init(this);
      }
      ...
      private function detachParachute() : void
      {
         ...
         if(this.parachute != null)
         {
            _loc1_ = ParachuteDetachAnimation(this.battleService.getObjectPool().getObject(ParachuteDetachAnimation));
      ...
      public function render(param1:int, param2:int) : void
      {
         ...
         this.bonusMesh.readPosition(origin);
         if(GoldBoxDiagnostics.shouldCaptureBonusPosition(GoldBoxDiagnostics.canonicalFromLong(this._bonusId)))
         {
            GoldBoxDiagnostics.captureBonusPosition(GoldBoxDiagnostics.canonicalFromLong(this._bonusId),origin.x,origin.y,origin.z);
         }
      ...
      {
         GoldBoxDiagnostics.lifecycle(GoldBoxDiagnostics.canonicalFromLong(this._bonusId),"LOCAL_OVERLAP_DETECTED","COLLISION");
         this.trigger.disable();
      ...
      public function onTouchGround() : void
      {
         this.bonusMesh.readPosition(_bonusPosition);
         GoldBoxDiagnostics.captureBonusPosition(GoldBoxDiagnostics.canonicalFromLong(this._bonusId),_bonusPosition.x,_bonusPosition.y,_bonusPosition.z,true);
         GoldBoxDiagnostics.lifecycle(GoldBoxDiagnostics.canonicalFromLong(this._bonusId),"GROUND_TOUCH","LANDING");
         this.detachParachute();
         if(!this.runNextController())
         {
            this.stopMovement();
      ...
      public function onLandingComplete() : void
      {
         this.bonusMesh.readPosition(_bonusPosition);
         GoldBoxDiagnostics.captureBonusPosition(GoldBoxDiagnostics.canonicalFromLong(this._bonusId),_bonusPosition.x,_bonusPosition.y,_bonusPosition.z,true);
         GoldBoxDiagnostics.lifecycle(GoldBoxDiagnostics.canonicalFromLong(this._bonusId),"LANDING_COMPLETE","COLLECTIBLE");
         this.stopMovement();
      }
      ...
      private function removeLight() : void
      {
         ...
         if(this.lightSource.parent != null)
         {
            ...
            _loc1_ = FadingLightEffect(this.battleService.getObjectPool().getObject(FadingLightEffect));
            _loc1_.init(this.lightSource,FADEOUT_TIME,FADEOUT_CURVE);
            this.battleService.addGraphicEffect(_loc1_);
         }
      }
