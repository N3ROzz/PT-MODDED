package alternativa.tanks.model.garage
{
   ...
   public class GarageModel extends GarageModelBase implements IGarageModelBase, ObjectLoadListener, Garage, ObjectUnloadPostListener, ITemporaryItemNotifyServiceListener, GaragePurchaseConfirmation, IResourceLoadingComplete
   {
      ...
      public function initMarket(param1:Vector.<IGameObject>) : void
      {
         ...
         {
            if(PaintRegistry.isVisualOnlyItem(_loc2_))
            {
               this.garageWindow.addItemToDepot(_loc2_);
            }
         }
      }
      ...
      {
         if(itemService.getCategory(param1.item) == ItemCategoryEnum.COLOR)
         {
            PaintRegistry.setSelectedPaint(itemService.getName(param1.item));
            this.applyGaragePreviewColor(param1.item);
         }
         this.mountItem(param1.item);
         if(PaintRegistry.isVisualOnlyItem(param1.item))
         {
            return;
         }
      ...
      private function applyGaragePreviewColor(param1:IGameObject) : void
      {
         var _loc2_:IColoring = null;
         if(!param1.hasModel(IColoring))
         {
            return;
         }
         _loc2_ = IColoring(param1.adapt(IColoring));
         if(_loc2_.getColoring() != null && _loc2_.getColoring().data != null)
         {
            tank3DViewer.setColor(_loc2_.getColoring().data);
         }
      }
